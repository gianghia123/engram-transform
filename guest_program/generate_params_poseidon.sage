# Modified script to generate parameters for POSEIDON, described and used in Grassi et al.
# (2019).
# This script differs from the original only in the output's format, which is a Rust array.
# Adhering to this script's licensing term, a copy of the license is included, and could be
# found in the folder licences/.
...
def print_round_constants(round_constants, n, field):
    ...
    rust_lines = ['"{0:#0{1}x}"'.format(entry, hex_length) for entry in round_constants]
    rust_array = (
        "pub const ROUND_CONSTANTS: [Fr; " + str(len(round_constants)) + "] = [\n"
        + "\n".join("    MontFp!(" + line + ")," for line in rust_lines)
        + "\n];"
    )

    print(rust_array)
    if write_file == True:
        FILE.write(rust_array + "\n")
...
def print_linear_layer(M, n, t):
    ...
    # Build a Rust 2D array: [[&str; t]; t]
    row_strings = []
    for i in range(0, t):
        if FIELD == 0:
            row_vals = ['MontFp!("{0:#0{1}x}")'.format(entry.integer_representation(), hex_length) for entry in M[i]]
        elif FIELD == 1:
            row_vals = ['MontFp!("{0:#0{1}x}")'.format(int(entry), hex_length) for entry in M[i]]
        row_strings.append("    [" + ", ".join(row_vals) + "]")

    rust_matrix = (
        "pub const MDS_MATRIX: [[Fr; " + str(t) + "]; " + str(t) + "] = [\n"
        + ",\n".join(row_strings)
        + "\n];"
    )
