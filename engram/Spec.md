...
3. If `is_merkle = 1`, each 2 consecutive chunks is a leaf of the Merkle tree. If there is a single node in any level, bump it to the next level on the tree (i.e. the parent of that node contain the value of itself.). If any leaf has 1 element, bump it to the next level as-is.2
